#https://github.com/yassouali/pytorch_segmentation/blob/master/models/deeplabv3_plus.py
...
class ResNet(nn.Module):
    def __init__(self, in_channels=3, output_stride=16, backbone='resnet18', pretrained=True):
...
class Decoder(nn.Module):
    def __init__(self, low_level_channels, num_classes_det,num_classes_seg,decoder_size='large'):
        ...
        if decoder_size=='large':
            self.output_det = nn.Sequential(
                ...
                nn.ReLU(inplace=True),
                nn.Dropout(0.1),
                nn.Conv2d(256, num_classes_det, 1, stride=1),
            )
            self.output_seg = nn.Sequential(
                ...
                nn.ReLU(inplace=True),
                nn.Dropout(0.1),
                nn.Conv2d(256, num_classes_seg, 1, stride=1),
            )
        elif decoder_size=='small':
            self.output_det = nn.Sequential(
                nn.Conv2d(48+256, 3, 3, stride=1, padding=1, bias=False),
            )
            self.output_seg = nn.Sequential(
                nn.Conv2d(48+256, 3, 3, stride=1, padding=1, bias=False),
            )
    ...
    def forward(self, x, low_level_features):
        ...
        det = self.output_det(x)
        seg = self.output_seg(x)
        return det,seg
...
class DeepLab(nn.Module):
    def __init__(self, num_classes_det,num_classes_seg, in_channels=3, backbone='resnet101', pretrained=True, 
                output_stride=16, freeze_bn=False, decoder_size='large',upsample=False,**_):
                
        super(DeepLab, self).__init__()
        assert ('xception' or 'resnet' in backbone)
        if 'resnet101' in backbone:
        ...
        self.decoder = Decoder(low_level_channels, num_classes_det,num_classes_seg,decoder_size=decoder_size)
    ...
    def forward(self, x):
        if self.upsample:
            ...
            H, W = x.size(2)//4, x.size(3)//4

        x, low_level_features = self.backbone(x)
        x = self.ASSP(x)
        det,seg = self.decoder(x, low_level_features)
